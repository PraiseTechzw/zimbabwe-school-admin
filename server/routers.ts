...
import { getExecutiveReport, getSystemHealth, listAuditLogs, listIntegrityIssues, listNotificationAudit, recordAuditEvent, reportToCsv, runStudentIdIntegrityAudit } from "./reporting";
...
export const appRouter = router({
  ...
  academic: router({
    ...
    createLearner: adminProcedure.input(learnerInput).mutation(async ({ input, ctx }) => { const result = await createLearner({ ...input, status: "ACTIVE" }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "CREATE", entity: "learner", entityId: result?.id, newValue: { studentId: result?.studentId, admissionNumber: result?.admissionNumber }, reason: "Learner created." }); return result; }),
    recordHistory: adminProcedure.input(historyInput).mutation(async ({ input, ctx }) => { const result = await recordAcademicHistory({ ...input, recordedByUserId: ctx.user.id }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "CREATE", entity: "learner_academic_history", entityId: result?.id, newValue: { learnerId: input.learnerId, academicYearId: input.academicYearId, termId: input.termId, formId: input.formId }, reason: "Academic history period recorded without overwriting prior history." }); return result; }),
    saveOLevelResults: protectedProcedure.input(oLevelInput).mutation(async ({ input, ctx }) => { const result = await saveOLevelResults(input); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "CREATE", entity: "o_level_results", entityId: result?.id, newValue: { learnerId: input.learnerId, examinationYear: input.examinationYear, candidateNumber: input.candidateNumber }, reason: "ZIMSEC result record created as preserved examination history." }); return result; }),
    ...
    enrolForm5: adminProcedure.input(z.object({ applicationId: z.number().int().positive(), academicYearId: z.number().int().positive(), termId: z.number().int().positive(), classId: z.number().int().positive().optional().nullable() })).mutation(async ({ input, ctx }) => { const result = await explicitlyEnrolForm5({ ...input, recordedByUserId: ctx.user.id }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "APPROVE", entity: "a_level_application", entityId: input.applicationId, newValue: { form5HistoryId: result?.id }, reason: "Form 5 enrolment explicitly approved after A-Level admission workflow." }); return result; }),
    progressForm6: adminProcedure.input(z.object({ learnerId: z.number().int().positive(), academicYearId: z.number().int().positive(), termId: z.number().int().positive(), classId: z.number().int().positive().optional().nullable() })).mutation(async ({ input, ctx }) => { const result = await progressToForm6({ ...input, recordedByUserId: ctx.user.id }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "TRANSFER", entity: "learner_academic_history", entityId: result?.id, newValue: { learnerId: input.learnerId, form: "Form 6" }, reason: "Form 6 progression created as a new academic history record." }); return result; }),
  }),
  finance: router({
    ...
    createInvoice: adminProcedure.input(invoiceInput).mutation(async ({ input, ctx }) => { const result = await createInvoice({ ...input, createdByUserId: ctx.user.id }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "CREATE", entity: "invoice", entityId: result?.id, newValue: { learnerId: input.learnerId, currency: input.currency, totalMinor: result?.totalMinor }, reason: "Invoice created." }); return result; }),
    recordPayment: adminProcedure.input(paymentInput).mutation(async ({ input, ctx }) => { const result = await recordPayment({ ...input, receivedByUserId: ctx.user.id }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: "CREATE", entity: "payment", entityId: result?.id, newValue: { learnerId: input.learnerId, receiptNumber: input.receiptNumber, amountMinor: input.amountMinor, currency: input.currency, method: input.method }, reason: "Payment recorded." }); return result; }),
    ...
    reconcilePayment: adminProcedure.input(z.object({ paymentId: z.number().int().positive(), externalReference: z.string().max(160).optional().nullable(), status: z.enum(["UNMATCHED", "MATCHED", "EXCEPTION", "REVERSED"]), exceptionReason: z.string().max(2000).optional().nullable() })).mutation(async ({ input, ctx }) => { const result = await reconcilePayment({ ...input, reconciledByUserId: ctx.user.id }); await recordAuditEvent({ userId: ctx.user.id, role: ctx.user.role, action: input.status === "REVERSED" ? "REVERSE" : "UPDATE", entity: "payment_reconciliation", entityId: input.paymentId, newValue: { status: input.status, externalReference: input.externalReference }, reason: input.exceptionReason ?? "Payment reconciliation updated." }); return result; }),
  }),
  welfare: router({
  ...
  reporting: router({
    executive: adminProcedure.input(z.object({ academicYearId: z.number().int().positive().optional(), termId: z.number().int().positive().optional(), formId: z.number().int().positive().optional(), classId: z.number().int().positive().optional(), gender: z.string().max(30).optional(), boardingStatus: z.enum(["BOARDING", "DAY_SCHOLAR"]).optional() }).optional()).query(async ({ input }) => getExecutiveReport(input ?? {})),
    exportExecutiveCsv: adminProcedure.input(z.object({ filters: z.object({ academicYearId: z.number().int().positive().optional(), termId: z.number().int().positive().optional(), formId: z.number().int().positive().optional(), classId: z.number().int().positive().optional(), gender: z.string().max(30).optional(), boardingStatus: z.enum(["BOARDING", "DAY_SCHOLAR"]).optional() }).optional() })).query(async ({ input }) => reportToCsv(await getExecutiveReport(input.filters ?? {}))),
    auditLogs: adminProcedure.input(z.object({ limit: z.number().int().positive().max(500).optional() }).optional()).query(async ({ input }) => listAuditLogs(input?.limit)),
    integrityAudit: adminProcedure.mutation(async () => runStudentIdIntegrityAudit()),
    integrityIssues: adminProcedure.input(z.object({ limit: z.number().int().positive().max(500).optional() }).optional()).query(async ({ input }) => listIntegrityIssues(input?.limit)),
    health: adminProcedure.query(async () => getSystemHealth()),
    notificationAudit: adminProcedure.input(z.object({ limit: z.number().int().positive().max(500).optional() }).optional()).query(async ({ input }) => listNotificationAudit(input?.limit)),
  }),
});
