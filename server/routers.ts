...
import { calculateInvoiceTotals, createFeeStructure, createInvoice, getFinanceData, recordPayment } from "./finance";
...
const feeStructureInput = z.object({ name: z.string().min(2).max(160), academicYearId: z.number().int().positive(), formId: z.number().int().positive().optional().nullable(), pathway: z.enum(["O_LEVEL", "A_LEVEL"]).optional().nullable(), currency: z.string().length(3), items: z.array(z.object({ code: z.string().min(1).max(40), name: z.string().min(2).max(160), description: z.string().max(1000).optional().nullable(), amountMinor: z.number().int().min(0), frequency: z.enum(["ONCE", "TERM", "ANNUAL"]), isMandatory: z.boolean() })).min(1) });
const invoiceInput = z.object({ learnerId: z.number().int().positive(), academicYearId: z.number().int().positive(), feeStructureId: z.number().int().positive().optional().nullable(), invoiceNumber: z.string().min(2).max(60), currency: z.string().length(3), discountMinor: z.number().int().min(0).optional(), dueDate: z.coerce.date().optional().nullable(), status: z.enum(["DRAFT", "ISSUED"]), notes: z.string().max(2000).optional().nullable(), lines: z.array(z.object({ feeItemId: z.number().int().positive().optional().nullable(), description: z.string().min(2).max(180), quantity: z.number().int().positive(), unitAmountMinor: z.number().int().min(0) })).min(1) });
const paymentInput = z.object({ learnerId: z.number().int().positive(), invoiceId: z.number().int().positive().optional().nullable(), receiptNumber: z.string().min(2).max(60), amountMinor: z.number().int().positive(), currency: z.string().length(3), method: z.enum(["CASH", "ECOCASH", "ZIPIT", "BANK_TRANSFER", "CARD", "OTHER"]), reference: z.string().max(120).optional().nullable(), status: z.enum(["PENDING", "CONFIRMED"]), paidAt: z.coerce.date().optional(), notes: z.string().max(2000).optional().nullable() });
...
export const appRouter = router({
  ...
  finance: router({
    overview: protectedProcedure.query(async () => getFinanceData()),
